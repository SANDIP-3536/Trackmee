<?php 
	...
	*/
	class Stop_model extends CI_Model
	{
		...
		{
			$query = $this->db->query('SELECT route_name,route_no FROM route where school_profile_id = 0 GROUP BY 1,2 ORDER BY 1');
		...
		function stop_details_with_route_type_1($data1)
		{
			...
			$route_type = $data1['route_type'];
			$query = $this->db->query('SELECT * FROM stop  JOIN (select route_id FROM route where route_no = '.$route_id.' and route_type = '.$route_type.' ) as routes on stop_route_id = routes.route_id
				where stop_expiry_date="9999-12-31"');
				return $query->result_array();
		}
		...
		function stop_details_with_route_type_2($data1)
		{
			$route_id = $data1['route_id'];
			$route_type = $data1['route_type'];
			$query = $this->db->query('SELECT * FROM stop  JOIN (select route_id FROM route where route_no = '.$route_id.' and route_type = '.$route_type.') as routes on stop_route_id = routes.route_id where stop_expiry_date="9999-12-31"'
				);
				return $query->result_array();
		}
