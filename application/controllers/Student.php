<?php
	...
	class Student extends CI_Controller
	{
		...
		function student_registration()
		{
			...
			$student['class_details'] =  $this->db->query("SELECT class.* FROM `class` join fees_type on fees_type_class_id = class_id and fees_type_school_profile_id = class_school_profile_id where class_school_profile_id =".$employee_school_profile_id." and class_expiry_date = '9999-12-31' group by fees_type_class_id")->result_array();
		...
		function add_student_registration()
		{
			...
			for ($i=0; $i < $cont; $i++) { 
				...
				if($this->email->send()){
					$check = $this->Enquiry_model->check_sms_active($school_admin[0]['school_school_profile_id']);
            ...
            for ($i=0; $i < $cnt; $i++) { 
            	...
            	$fee_waiver_details['fee_waiver_amount'] = $fee_type['fee_waiver_amount'][$i];
		...
		function add_another_parent()
		{
			...
			if($this->email->send()){
				if($school_admin[0]['school_authentication_details_sms'] == 1)
				{
					...
					$this->Enquiry_model->set_count($school_admin[0]['school_institute_profile_id']);
					$sent['sent_sms_type'] = 2;
					$sent['sent_sms_sub_type'] = 16;
		...
		{	
			...
			$data['fee_details'] = $this->db->query("(SELECT * FROM total_fees join fees_type on total_fee_type_id = fees_type_id and total_fee_AY_id = fees_type_AY_id join fee_category on fee_category_id = fees_type_fee_category_id where total_fee_student_profile_id =".$id." and total_fee_school_profile_id = ".$employee_school_profile_id." and fees_type_name NOT IN('Default') ORDER BY fees_type_name)
												union
												(SELECT * FROM total_fees join fees_type on total_fee_type_id = fees_type_id and total_fee_AY_id = fees_type_AY_id join fee_category on fee_category_id = fees_type_fee_category_id where total_fee_student_profile_id =".$id." and total_fee_school_profile_id = ".$employee_school_profile_id." and fees_type_name IN('Default') ORDER BY fees_type_name)")->result_array();
			...
			$data['total_fee_update'] = $this->db->query("(SELECT * FROM fees_type join fee_category on fees_type_fee_category_id=fee_category_id and fees_type_AY_id=".$AY_id." left join total_fees on total_fee_type_id=fees_type_id and fees_type_AY_id=total_fee_AY_id and total_fee_student_profile_id=".$id." where fees_type_class_id=".$class_id." and fees_type_expiry_date='9999-12-31' and fees_type_school_profile_id=".$employee_school_profile_id." and fees_type_name NOT IN('Default') ORDER BY fees_type_name)
														union
														(SELECT * FROM	fees_type join fee_category on fees_type_fee_category_id=fee_category_id and fees_type_AY_id=".$AY_id." left join total_fees on total_fee_type_id=fees_type_id and fees_type_AY_id=total_fee_AY_id and total_fee_student_profile_id=".$id." where fees_type_class_id=".$class_id." and fees_type_expiry_date='9999-12-31' and fees_type_school_profile_id=".$employee_school_profile_id." and fees_type_name IN('Default') ORDER BY fees_type_name)")->result_array();
			$school['user_profile_id'] = $school_admin[0]['employee_profile_id'];
		...
		function update_student_fee_structure()
		{
            ...
            for ($i=0; $i < $cnt; $i++) { 
            	...
            	$fee_waiver_details['fee_waiver_amount'] = $fee_type['fee_waiver_amount'][$i];
            	$fee_waiver_details['fee_waiver_name'] = $fee_type['fee_waiver_name'][$i];
            	$fee_waiver_details['fee_waiver_student_profile_id'] = $student_profile_id;
            	$fee_waiver_details['fee_waiver_effective_date'] = date('Y-m-d');
            	$fee_waiver_details['fee_waiver_school_profile_id'] = $school_admin[0]['employee_school_profile_id'];
            	$fee_waiver_details['fee_waiver_AY_id'] = $school_admin[0]['school_AY_id'];
            	$this->db->insert('total_fees',$fee_type_details);
