...
class Institute extends CI_Controller {
	...
	function institute_Details()
	{
		...
		$institute['all_employee'] = $this->db->query("SELECT employee.*,institute_name from employee join institute on institute_profile_id = employee_client_profile_id where employee_type = 2")->result_array(); 
	...
	function new_institute_registration()
	{
		...
		$panic = $this->input->post('institute_panic_notifi');
		if($panic == 'on'){
			$institute_details['institute_panic_notifi'] = "1";
		}else{
			$institute_details['institute_panic_notifi'] = "0";
		}
		$stop_SMS = $this->input->post('institute_stop_sms');
		if($stop_SMS == 'on'){
			$institute_details['institute_stop_sms'] = "1";
		}else{
			$institute_details['institute_stop_sms'] = "0";
		}
		$dest = $this->input->post('institute_dest_sms');
		if($dest == 'on'){
			$institute_details['institute_dest_sms'] = "1";
		}else{
			$institute_details['institute_dest_sms'] = "0";
		}
		$auth = $this->input->post('institute_auth_sms');
		if($auth == 'on'){
			$institute_details['institute_auth_sms'] = "1";
		}else{
			$institute_details['institute_auth_sms'] = "0";
		}
	...
	function update_details_institute()
	{
		...
		$panic = $this->input->post('institute_panic_notifi');
		if($panic == 'on'){
			$institute_update['institute_panic_notifi'] = "1";
		}else{
			$institute_update['institute_panic_notifi'] = "0";
		}
		$stop_SMS = $this->input->post('institute_stop_sms');
		if($stop_SMS == 'on'){
			$institute_update['institute_stop_sms'] = "1";
		}else{
			$institute_update['institute_stop_sms'] = "0";
		}
		$dest = $this->input->post('institute_dest_sms');
		if($dest == 'on'){
			$institute_update['institute_dest_sms'] = "1";
		}else{
			$institute_update['institute_dest_sms'] = "0";
		}
		$auth = $this->input->post('institute_auth_sms');
		if($auth == 'on'){
			$institute_update['institute_auth_sms'] = "1";
		}else{
			$institute_update['institute_auth_sms'] = "0";
		}
	...
	function add_institute_user()
	{
		...
		if ($mobile != 0) {
			...
			$institute_employee['employee_client_profile_id'] = $this->input->post('institute_profile_id');
			$institute_details['institute_name'] = $this->input->post('institute_name');
			...
			$this->email->message("Hi,<br>Your profile has been created with ".$institute_details['institute_name'].". Your credentials are as follows:<br>    Username: ".$institute_credential['credential_username']."<br>   Password: ".$institute_credential1['credential_password1']."<br><br>Regards,<br>Team TrackMee");
			...
			if($this->email->send()){
				...
				if($check[0]['institute_auth_sms'] == 1 && $check[0]['institute_sms_credit'] > 0)
				{
					...
					$sent['sent_sms_client_profile_id'] = $num['profile_id'];
	...
	function institute_change_password()
	{
		$institute_admin = $this->session->userdata('Institute');
		$employee_details = $this->Institute_model->employee_details($institute_admin);
		$employee_password['credential_profile_id'] = $this->input->post('user_profile_id');
		$institute_employee['employee_email_id'] = $employee_details[0]['employee_email_id'];
		$institute_employee['employee_mobile_number'] = $employee_details[0]['employee_pri_mobile_number'];
		...
		$this->email->initialize($config);
		$this->email->from('ino-replt@trackmee.syntech.co.in');	
		$this->email->to(''.$institute_employee['employee_email_id'].'');
